...
console.log("\nKathanika Media V70 — Mobile Lighthouse Performance Verification\n");
const pkg=JSON.parse(read("package.json").replace(/^\uFEFF/, ""));
check(pkg.name === "kathanika-media-v70-frontend-only-production", "Package identifies V70 frontend-only production build");
check(pkg.version === "70.0.0", "Package version is 70.0.0");
...
if(failures){console.error(`\nV70 Lighthouse verification failed with ${failures} issue(s).`);process.exit(1)}
console.log("\nV70 Lighthouse verification passed.");
